...
from detection_fusion.core import associate, covariance, fuse_position, transform_matrix
...
class DetectionFusion(Node):
            ...
            if camera is not None and self.info is not None:
                if self.info.header.frame_id == camera.header.frame_id:
                    try:
                        ...
                        self.emit(lidar, (camera, matrix), 'fused')
    ...
    def emit(self, lidar, camera_data, reason):
        ...
        matches = []
        colored = 0
        if camera_data is not None and output.cones:
            camera, matrix = camera_data
        ...
        self.status.publish(String(data=json.dumps({
            ...
            'lidar_cones': len(output.cones), 'matches': len(matches), 'colored': colored})))
